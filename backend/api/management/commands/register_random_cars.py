...
def generate_random_plate():
    ...
    if plate_format == 0:
        plate += "".join([chr(char) for char in random.sample(range(65, 91), 3)])
        plate += " "
        plate += "".join([str(num) for num in random.sample(range(0, 10), 3)])
    else:
        plate += "".join([chr(char) for char in random.sample(range(65, 91), 2)])
        plate += " "
        plate += "".join([str(num) for num in random.sample(range(0, 10), 3)])
        plate += " "
        plate += "".join([chr(char) for char in random.sample(range(65, 91), 2)])
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):        
        ...
        self.stdout.write(self.style.SUCCESS('It will insert {} recors'.format(number)))        
